...
def zone_field_pipeline(field_slug: str, output_dir: Path) -> None:
    ...
    zones = make_zones(field, ZoningConfig(
            3,
            3,
            pricing,
        ))
    ...
    kpis = {
        ...
        "base_revenue": round(pricing.calculate_price(whole_protein/whole_yield, whole_yield),2)
    }
    if sol.solve_info:
        kpis.update(
            {
    ...
    with open(output_dir / f"{field_slug}_zones.csv", "w") as file:
        ...
        for z in sol.zones:
            writer.writerow(
                {"x1": z.box[0][0], "y1": z.box[0][1], "x2":z.box[1][0], "y2": z.box[1][1], "score":z.score}
            )
